...
## プロジェクト構成とモジュール配置
アプリ本体は `src/`、サーバー側（Cloudflare Worker）は `worker/` 配下にあります。
- `src/components/`: UI タブや React コンポーネント（`DashboardTab.tsx`、`InputTab.tsx` など）
- `src/domain/`: ドメインモデル、スキーマ、保存処理、レベル計算、同期ロジック
- `src/hooks/`: カスタムフック（`useSync.ts` など）
- `src/lib/`: 共有ユーティリティ（`cn()`、better-auth クライアント）
- `src/utils/`: 画面表示向けの補助処理
- `src/data/characterMaster.json`: アプリで利用するマスターデータ
- `src/test/setup.ts`: フロント側 Vitest の共通セットアップ
- `worker/`: Cloudflare Worker（better-auth 認証、`/api/data` 同期 API）。**`window`/`localStorage` に依存するモジュールを import しないこと**（DOM 非依存の `src/domain` モジュールのみ可）
- `migrations/`: D1 マイグレーション SQL
- `test/worker/`: Worker 側テスト（`@cloudflare/vitest-pool-workers`）
...
## UI コンポーネント規約
- **デザインシステム準拠**: UI・デザイン（色・形・タイポグラフィ・コンポーネント規約）の実装はリポジトリルートの `DESIGN.md`（デザインシステム「スレートナイト」）に従う

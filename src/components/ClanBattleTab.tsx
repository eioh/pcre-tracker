...
export function ClanBattleTab({ masterCharacters, state, onChange }: ClanBattleTabProps) {
  ...
  const handleAddMonthGroup = (year: number, month: number): void => {
    updateClanBattle((previous) => {
      if (previous.groups.some((group) => group.year === year && group.month === month)) {
        return previous;
      }
      return {
        groups: [...previous.groups, createClanBattleMonthGroup(year, month)],
      };
    });
  };
  ...
  // 指定した年月グループの末尾へ編成を追加する。
  const handleAddFormation = (groupId: string, name: string): void => {
    const formation = createClanBattleFormation(name);
    updateClanBattle(
      (previous) => ({
  ...
  return (
    <section className="grid gap-4 xl:grid-cols-[340px_minmax(0,1fr)]">
      <ClanBattleSidebar
        groups={state.clanBattle.groups}
        selectedFormationId={selectedFormation?.id ?? null}
        onSelectFormation={setSelectedFormationId}
        onAddMonthGroup={handleAddMonthGroup}

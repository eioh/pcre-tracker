...
describe("InputProgressList", () => {
  ...
  it("専用装備未実装キャラはサマリーの専用1・専用2を「-」で表示しmaxed扱いにしない", () => {
    ...
    expect(ue1.className).not.toContain("text-maxed-green");
    expect(ue2.className).toContain("text-muted");
    expect(ue2.className).not.toContain("text-maxed-green");
  });
  ...
  it("実装段階の最大まで強化済みの項目はサマリーをmaxed色で表示する", () => {
    ...
    expect(screen.getByText("☆6").className).toContain("text-maxed-green");
    expect(screen.getByText("CR15").className).toContain("text-maxed-green");
    expect(screen.getByText("専用1: SP").className).toContain("text-maxed-green");
    expect(screen.getByText("専用2: 5").className).toContain("text-maxed-green");
  });
  ...
  it("☆6未実装キャラは☆5でサマリーの☆をmaxed色で表示する", () => {
    ...
    expect(screen.getByText("☆5").className).toContain("text-maxed-green");
  });
  ...
  it("最大未達の項目はサマリーをtext-mutedのまま表示する", () => {
    ...
    for (const element of [
      ...
      expect(element.className).not.toContain("text-maxed-green");
    }
  });

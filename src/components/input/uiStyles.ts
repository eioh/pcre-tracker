export const panelClass =
  "rounded-[18px] border border-white/30 bg-linear-to-br from-section-from to-section-to p-5 shadow-panel";
export const inputToolbarClass = "grid grid-cols-1 gap-3 lg:grid-cols-[repeat(auto-fit,minmax(180px,1fr))]";
export const sectionLabelClass = "mb-2 text-sm font-semibold text-sub";
export const filterSeparatorClass = "my-3 h-px bg-filter-separator";
export const memoryCalcSectionClass = "mt-0.5 pt-1";
export const memoryCalcGridClass = "grid grid-cols-1 gap-3 md:grid-cols-3";
export const tableWrapClass =
  "h-[70vh] overflow-auto rounded-[14px] border border-table-wrap-border bg-table-wrap-bg [overscroll-behavior-x:contain] [overscroll-behavior-y:auto] [scrollbar-gutter:stable] [contain:layout_paint_size]";
export const sortButtonClass =
  "inline-flex cursor-pointer items-center gap-1.5 whitespace-nowrap border-0 bg-transparent p-0 text-inherit hover:text-sort-hover";
export const sortIndicatorClass = "min-w-[0.85em] text-[0.72rem] text-accent";
export const tableSwitchClass = "inline-flex items-center gap-2 whitespace-nowrap text-sm";
export const sourceChipBaseClass = "rounded-full border px-2 py-0.5 text-[0.72rem] whitespace-nowrap";
export const characterNameCellLayoutClass = "grid gap-1";
export const characterTagLineClass = "flex flex-wrap items-center gap-2 text-[0.72rem] font-semibold leading-none";
